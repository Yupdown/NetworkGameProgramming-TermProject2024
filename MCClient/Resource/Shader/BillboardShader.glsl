...
void main()
{
    ...
    // Billboard Transform
    vec3 x = normalize(vec3(uView * uModel * vec4(1, 0, 0, 0)));
    vec3 y = normalize(vec3(uView * uModel * vec4(0, 1, 0, 0)));
    vec3 z = cross(x, y);
    ...
    FragPos = vec3(uView * uModel * billboardMatrix * vec4(aPos, 1.0f));
    gl_Position = uProj * uView * uModel * billboardMatrix * vec4(aPos, 1.0f);
}
