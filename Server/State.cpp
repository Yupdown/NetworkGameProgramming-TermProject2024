...
ServerCore::S_ptr<class ClientSession >FindClosestValidSession(const glm::vec3& current_position,const float th_hold)
{
	float min_distance = std::numeric_limits<float>::max();
	ServerCore::S_ptr<class ClientSession> closest_session = nullptr;
...
MON_STATE Patrol::Update(const float DT)
{
	...
	if ((cur_target == m_fsm->target) && m_fsm->target)
	{
		...
		if (m_accTime >= 1.5f)
		{
			entity_movement->dest = m_fsm->target->current_position;
