...
void main()
{
    ...
    float x = aPos.x / 32.0f;
    worldPos.y += sin(abs(x) - pad5 * 4.0f) * max(0.2f, abs(x) - 0.5f);
...
void main()
{
    ...
    float fogFactor = clamp((distance(uViewPos, fragPos) - 200.0) / 50.0, 0.0, 1.0);
