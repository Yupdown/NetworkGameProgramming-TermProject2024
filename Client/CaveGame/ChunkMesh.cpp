...
void ChunkMesh::ReConstructVertex(vector<Vertex>& v1, vector<Vertex>& v2, shared_ptr<Mesh> mesh, const GLuint idx) noexcept
{
    ...
    memcpy(cache_vert + vert_end_after,
        cache_vert + vert_end_before,
        (m_numOfVertices - vert_end_before) * sizeof(*cache_vert));
    ::memcpy(cache_vert + vert_off, v2.data(), sizeof(Vertex) * v2Size);
    const auto& mesh_mat = m_vecChunkInfo[idx].worldMat;
    std::for_each(std::execution::par_unseq, cache_vert + vert_off, cache_vert + vert_end_after, [mesh_mat](Vertex& vert) {
        vert.position = mesh_mat * glm::vec4{ vert.position,1.f };
        });
...
{
    i1.shrink_to_fit();
    std::for_each(std::execution::par_unseq, m_vecChunkInfo.begin(), m_vecChunkInfo.end(),[&](const auto& chunk) {
...
void ChunkMesh::OnChunkMeshChanged(MCTileChunk* const pChunk, int chunkX, int chunkZ)noexcept
{
    if (m_vecChunkInfo.empty())
        return;
    if (!m_mapChunkToIndex.contains(pChunk))
        return;
    const auto idx = m_mapChunkToIndex[pChunk];
    shared_ptr<Mesh> mesh = MCTilemapMeshGenerator::CreateMeshFromChunk(m_pTileMapForReDrawMesh, chunkX, chunkZ);
    
    auto& v1 = m_vecChunkInfo[idx].refMesh->GetVertices();
    auto& i1 = m_vecChunkInfo[idx].refMesh->GetIndicies();
    if (v1.empty() || i1.empty())
        return;
    ...
    m_bDirty = true;
    i1.swap(i2);
    //ReConstructVertex(v1, v2, std::move(mesh), idx);
    //ReConstructIndex(i1);
    Mgr(ThreadMgr)->Enqueue([this, &v1, &v2, mesh = std::move(mesh), idx]()noexcept {ReConstructVertex(v1, v2, std::move(mesh), idx); });
    Mgr(ThreadMgr)->Enqueue([this, &i1]()noexcept { ReConstructIndex(i1); });
